...
namespace PhpQa\Console\Command;

use Exception;
use PhpQa\Configuration\Resolver\TaskConfigResolver;
    ...
    public function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($this->filesystem->exists($configFile) && !$force) {
            if (!$input->getOption('silent')) {
                $output->writeln('<fg=yellow>PhpQa is already configured!</fg=yellow>');
            }
        ...
        if (!$configuration) {
            $output->writeln('<fg=yellow>Skipped configuring PhpQa. Using default configuration.</fg=yellow>');
        ...
        if (!$input->getOption('silent')) {
            $output->writeln('<fg=green>PhpQa is configured and ready to kick ass!</fg=green>');
        }
